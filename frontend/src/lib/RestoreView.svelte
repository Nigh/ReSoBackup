...
async function pickFile() {
  try {
    ...
    if (result) {
      ...
      fileError = '';
      try {
        const encrypted = await BackupService.ValidateRestoreFile(result);
        fileValid = true;
        needsPassword = encrypted;
        if (!encrypted) {
          status = 'info';
          statusMsg = getT().noPasswordNeeded;
        }
      } catch (e) {
        fileValid = false;
        needsPassword = false;
        ...
        status = 'error';
        statusMsg = getT().invalidFile + ': ' + fileError;
      }
    }
  } catch (e) {
    console.error(e);
...
<div class="space-y-6 max-w-2xl mx-auto">
  ...
  {#if status}
    <div class="alert {status === 'success' ? 'alert-success' : status === 'info' ? 'alert-info' : 'alert-error'}">
      {#if status === 'success'}
  ...
  <button
    ...
    onclick={doRestore}
    disabled={loading || !inputPath || !fileValid || (needsPassword && !password)}
  >
    {#if loading}
